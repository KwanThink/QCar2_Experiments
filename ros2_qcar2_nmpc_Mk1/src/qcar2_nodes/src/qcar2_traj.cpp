...
// Identify the forward/reverse car's direction
...
double selectAutoDirectionSign(
  const qcar2_nmpc::Pose2D & start_pose,
  const qcar2_nmpc::Pose2D & goal_pose)
{
...
{
  if (points.size() < 2) {
    throw std::runtime_error("The QCar2 NMPC trajectory generator expects at least two waypoints.");
  }
