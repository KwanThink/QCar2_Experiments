...
: enabled_(enabled),
  ...
  obstacle_map_file_(std::move(obstacle_map_file))
{
}
...
{
  run_log_.clear();
  full_reference_.clear();
  solver_failures_ = 0;
  run_exported_ = false;
}

// Store the complete active reference for export even if the run stops early.
void ResultLogger::setFullReferenceTrajectory(
  const qcar2_flmpc::ReferenceTrajectory & trajectory)
{
  full_reference_ = trajectory.samples;
}
...
// Append one measured state, reference, command, and solver-complexity sample.
  ...
  double solve_time)
{
  ...
  sample.slack_value = solver_result.slack_value;
  sample.solver_success = solver_result.success;
  sample.solver_status = solver_result.status_name;
  sample.number_of_config_obstacles = solver_result.number_of_config_obstacles;
  sample.active_obstacles = solver_result.active_obstacles;
  sample.active_edges = solver_result.active_edges;
  sample.binary_variables = solver_result.binary_variables;
  sample.total_binary_variables = solver_result.total_binary_variables;
  run_log_.push_back(std::move(sample));
}

// Create and return the next available OAMPC Mk1c run directory.
  ...
  const rclcpp::Logger & logger) const
{
  ...
  if (source_path.empty()) {
    try {
      ...
      source_path =
        std::filesystem::path(package_share_directory) / "config" / "qcar2_oampc.yaml";
    } catch (const std::exception & exception) {
      RCLCPP_WARN(
        logger,
        "Run data were saved, but the OAMPC source configuration could not be located: %s",
        exception.what());
      return;
    }
  }
  ...
  if (!std::filesystem::is_regular_file(source_path, error_code)) {
    RCLCPP_WARN(
      logger,
      "Run data were saved, but the OAMPC source configuration does not exist: %s",
  ...
  if (error_code) {
    RCLCPP_WARN(
      logger,
      "Run data were saved, but the OAMPC configuration could not be copied to %s: %s",
...
void ResultLogger::copyObstacleMap(
  const std::string & run_directory,
  const rclcpp::Logger & logger) const
{
  const std::filesystem::path source_path(obstacle_map_file_);
  std::error_code error_code;
  if (!std::filesystem::is_regular_file(source_path, error_code)) {
    RCLCPP_ERROR(
      logger,
      "Run data were saved, but the obstacle geometry snapshot is missing: %s",
      source_path.string().c_str());
    return;
  }

  const std::filesystem::path destination_path =
    std::filesystem::path(run_directory) / "obstacle_map_oampc_Mk1c.yaml";
  std::filesystem::copy_file(
    source_path,
    destination_path,
    std::filesystem::copy_options::overwrite_existing,
  ...
  if (error_code) {
    RCLCPP_ERROR(
      logger,
      "Run data were saved, but the obstacle geometry could not be copied to %s: %s",
      destination_path.string().c_str(),
      error_code.message().c_str());
  }
}

// Export CSV files, metrics, snapshots, and SVG plots for the active run.
void ResultLogger::exportRunResults(
  ...
  const rclcpp::Logger & logger)
{
  ...
  try {
    ...
    RCLCPP_ERROR(logger, "Failed to create OAMPC result directory: %s", exception.what());
    return;
  }

  const std::filesystem::path result_path(result_directory);
  const auto open_file = [&result_path](const std::string & file_name) {
  ...
  {
    auto file = open_file("states_oampc_Mk1c.csv");
    file << "t,X,Y,psi,vx\n";
    for (const auto & sample : run_log_) {
      file << sample.t << ',' << sample.x << ',' << sample.y << ','
           << sample.psi << ',' << sample.vx_measured << '\n';
    }
  }
  {
    auto file = open_file("controls_oampc_Mk1c.csv");
    file << "t,desired_speed,delta,ax\n";
    for (const auto & sample : run_log_) {
      file << sample.t << ',' << sample.vx_command << ',' << sample.delta_command << ','
           << sample.ax_command << '\n';
    }
  }
  {
    auto file = open_file("reference_used_oampc_Mk1c.csv");
    file << "t,X_ref,Y_ref,psi_ref,vx_ref\n";
    for (const auto & sample : run_log_) {
      file << sample.t << ',' << sample.x_ref << ',' << sample.y_ref << ','
           << sample.psi_ref << ',' << sample.vx_ref << '\n';
    }
  }
  {
    auto file = open_file("full_reference_oampc_Mk1c.csv");
    file << "t,X_ref,Y_ref,psi_ref,vx_ref,delta_ref,ax_ref\n";
    for (const auto & sample : full_reference_) {
      file << sample.t << ',' << sample.x << ',' << sample.y << ','
           << sample.psi << ',' << sample.vx << ',' << sample.delta << ',' << sample.ax << '\n';
    }
  }
  {
    auto file = open_file("reference_controls_oampc_Mk1c.csv");
  ...
  {
    auto file = open_file("virtual_inputs_oampc_Mk1c.csv");
    file << "t,v1,v2\n";
    for (const auto & sample : run_log_) {
  ...
  {
    auto file = open_file("solve_times_oampc_Mk1c.csv");
    file << "t,solve_time,solver_success,solver_status,slack_value,number_of_config_obstacles,"
            "active_obstacles,active_edges,binary_variables,total_binary_variables\n";
    for (const auto & sample : run_log_) {
      file << sample.t << ',' << sample.solve_time << ','
           << (sample.solver_success ? 1 : 0) << ','
           << quoteCsvString(sample.solver_status) << ',' << sample.slack_value << ','
           << sample.number_of_config_obstacles << ',' << sample.active_obstacles << ','
           << sample.active_edges << ',' << sample.binary_variables << ','
           << sample.total_binary_variables << '\n';
    }
  }
  ...
  for (const auto & sample : run_log_) {
    ...
    max_solve_time = std::max(max_solve_time, sample.solve_time);
  }
  ...
  const double count = static_cast<double>(run_log_.size());
  {
    ...
    file << "  \"max_solve_time\": " << max_solve_time << "\n";
    file << "}\n";
  }

  copySourceConfig(result_directory, logger);
  copyObstacleMap(result_directory, logger);
  ...
  try {
    const std::string package_share_directory =
      ament_index_cpp::get_package_share_directory("qcar2_nodes");
    plot_script_path =
      (std::filesystem::path(package_share_directory) / "scripts" / "plot_oampc_results.py").string();
  } catch (const std::exception & exception) {
    RCLCPP_WARN(
      logger,
  ...
  RCLCPP_INFO(logger, "Saved OAMPC Mk1c run results to %s", result_directory.c_str());
}
...
std::string ResultLogger::quoteCsvString(const std::string & value)
{
  std::string quoted = "\"";
  for (const char character : value) {
    if (character == '\"') {
