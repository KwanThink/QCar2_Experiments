...
def generate_launch_description():
    ...
    declare_use_namespace_cmd = DeclareLaunchArgument(
        'use_namespace',
        default_value='False',
        description='Whether to apply a namespace to the localization + NMPC stack',
    )
    ...
    declare_localization_params_file_cmd = DeclareLaunchArgument(
        ...
        default_value=os.path.join(bringup_dir, 'config', 'qcar2_localization_nmpc.yaml'),
        description='Full path to the localization parameter file',
    )

    declare_nmpc_params_file_cmd = DeclareLaunchArgument(
        'nmpc_params_file',
        default_value=os.path.join(bringup_dir, 'config', 'qcar2_nmpc.yaml'),
        description='Full path to the QCar2 NMPC parameter file',
    )
    ...
    qcar2_nmpc_controller_node = Node(
        package='qcar2_nodes',
        executable='qcar2_nmpc_controller',
        name='qcar2_nmpc_controller',
        output='screen',
        parameters=[
            nmpc_params_file,
            {'source_config_file': nmpc_params_file},
            {'use_sim_time': use_sim_time},
        ],
        arguments=['--ros-args', '--log-level', log_level],
    )

    nmpc_group = GroupAction([
        PushRosNamespace(
    ...
    ld.add_action(declare_nmpc_params_file_cmd)
    ld.add_action(declare_autostart_cmd)
    ld.add_action(declare_log_level_cmd)
    ld.add_action(declare_device_type_cmd)

    ld.add_action(nmpc_group)
