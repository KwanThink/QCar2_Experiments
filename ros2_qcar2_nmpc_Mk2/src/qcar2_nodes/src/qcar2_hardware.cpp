...
public:
    ...
    : Node("qcar2")
    {
        ...
        this->declare_parameter("led_color_id", 4, param_desc);
...
private:
    ...
    void speed_controller()
    {
        ...
        if (desired_speed != 0)
        {
            ...
            const double measured_acceleration = (measured_speed - prior_measured_speed)/dt;
            const double acceleration_error = desired_acceleration - measured_acceleration;
            
            motor_speed_cmd = motor_speed_cmd + (speed_error * kp + acceleration_error * kd) * km/battery_voltage;
            
            prior_measured_speed = measured_speed;
    ...
    {
        const size_t motor_commands_size = motor_commands.motor_names.size();

        if (motor_commands_size != motor_commands.values.size())
        {
            RCLCPP_WARN(
                this->get_logger(),
                "In %s - motor_names size and values size mismatch...ignore",
                __FUNCTION__);
            return;
        }
        ...
        {
            const std::string &name = motor_commands.motor_names[i];
            const double value = motor_commands.values[i];

            if (name == "steering_angle")
            {
                desired_steering = value;
            }
            else if (name == "desired_speed" || name == "motor_throttle")
            {
                desired_speed = value;
            }
            ...
            else
            {
                RCLCPP_WARN(
                    this->get_logger(),
                    "In %s - MotorCommands command_name %s is invalid...ignoring",
                    __FUNCTION__,
                    name.c_str());
            }
        }
    ...
    double desired_speed= 0;            // vx_cmd from NMPC [m/s]
    double desired_steering = 0;        // steering angle from NMPC [rad]
    double desired_acceleration = 0.0;  // ax_cmd from NMPC [m/s^2]
    double motor_speed_cmd = 0;
    double prior_measured_speed = 0.0;
