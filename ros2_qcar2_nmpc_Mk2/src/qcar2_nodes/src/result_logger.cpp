...
// Create and return the next available result directory for this Mk2 shape.
  ...
  const rclcpp::Logger & logger) const
{
  ...
  if (source_path.empty()) {
    try {
      ...
      source_path =
        std::filesystem::path(package_share_directory) / "config" / "qcar2_nmpc.yaml";
    } catch (const std::exception & exception) {
      RCLCPP_WARN(
        logger,
        "Run data were saved, but the NMPC source configuration could not be located: %s",
        exception.what());
      return;
    }
  }
  ...
  if (!std::filesystem::is_regular_file(source_path, error_code)) {
    RCLCPP_WARN(
      logger,
      "Run data were saved, but the NMPC source configuration does not exist: %s",
  ...
  if (error_code) {
    RCLCPP_WARN(
      logger,
      "Run data were saved, but the NMPC configuration could not be copied to %s: %s",
...
void ResultLogger::exportRunResults(const std::string & stop_reason, const rclcpp::Logger & logger)
{
  ...
  try {
    ...
    RCLCPP_ERROR(logger, "Failed to create NMPC result directory: %s", exception.what());
    return;
  }
  ...
  try {
    const std::string package_share_directory =
      ament_index_cpp::get_package_share_directory("qcar2_nodes");
    plot_script_path =
      (std::filesystem::path(package_share_directory) / "scripts" / "plot_nmpc_results.py").string();
  } catch (const std::exception & exception) {
    RCLCPP_WARN(
      logger,
  ...
  if (plot_status != 0) {
    ...
    RCLCPP_INFO(logger, "Saved NMPC run results to %s", result_directory.c_str());
  }
}
