...
// Create and return the next available run_FLMPC_Mk2_x directory.
...
void ResultLogger::exportRunResults(const std::string & stop_reason, const rclcpp::Logger & logger)
{
  ...
  {
    auto file = open_file("states_flmpc_Mk2.csv");
    file << "t,X,Y,psi,vx\n";
  ...
  {
    auto file = open_file("controls_flmpc_Mk2.csv");
  ...
  {
    auto file = open_file("reference_used_flmpc_Mk2.csv");
  ...
  {
    auto file = open_file("reference_controls_flmpc_Mk2.csv");
  ...
  {
    auto file = open_file("virtual_inputs_flmpc_Mk2.csv");
    file << "t,v1,v2\n";
    for (const auto & sample : run_log_) {
  ...
  {
    auto file = open_file("solve_times_flmpc_Mk2.csv");
