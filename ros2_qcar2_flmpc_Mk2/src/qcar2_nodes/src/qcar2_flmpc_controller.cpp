...
QCar2FLMPCController::QCar2FLMPCController(): Node("qcar2_flmpc_controller")
{
  ...
  try {
    ...
    RCLCPP_FATAL(
      this->get_logger(),
      "Invalid fixed-reference trajectory configuration: %s",
      exception.what());
    throw;
  }
  ...
  path_publish_timer_ = this->create_wall_timer(
    ...
    std::bind(&QCar2FLMPCController::pathPublishTimerCallback, this));

  publishZeroCommand();
  RCLCPP_INFO(
    this->get_logger(),
    "qcar2_flmpc_controller loaded the fixed reference path and is waiting for map -> base_link and a /goal_pose trigger.");
}

// Declare and load controller, solver, and fixed-reference trajectory parameters.
void QCar2FLMPCController::declareAndLoadParameters()
{
  ...
  this->declare_parameter<std::vector<double>>("R", {18.0, 18.0});
...
// Stop the current run and create the predefined active trajectory from the current TF pose.
void QCar2FLMPCController::goalCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
{
  ...
  active_trajectory_ = ReferenceTrajectory{};
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "2D Goal trigger accepted. Active FLMPC Mk2 trajectory is ready: %zu samples, %.3f s duration. The reference clock will start at |vx| >= %.3f m/s.",
    active_trajectory_.samples.size(),
    active_trajectory_.duration(),
    launch_velocity_threshold_);
}
...
void QCar2FLMPCController::controlTimerCallback()
{
  ...
  if (!reference_clock_started_ && std::abs(current_state.vx) >= launch_velocity_threshold_) {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Launch synchronization completed at |vx| = %.3f m/s. The fixed-reference clock starts now.",
      std::abs(current_state.vx));
  }

  const double reference_elapsed_time = reference_clock_started_ ?
    (current_time - reference_start_time_).seconds() : 0.0;
  ...
  const std::size_t elapsed_reference_index = static_cast<std::size_t>(
    std::max(
      0.0,
      std::round(reference_elapsed_time / flmpc_parameters_.sample_time)));
  const std::size_t reference_index = std::min(
    elapsed_reference_index,
    last_reference_index);

  const bool reference_finished =
    reference_clock_started_ && reference_elapsed_time >= active_trajectory_.duration();
  ...
  if (!solver_result.success) {
    ...
    RCLCPP_ERROR_THROTTLE(
      ...
      *this->get_clock(),
      1000,
      "acados FLMPC solver failed with status %d. Publishing zero command.",
  ...
  const double steering_angle = clip(
    solver_result.delta,
    flmpc_parameters_.delta_min,
    flmpc_parameters_.delta_max);
  const double longitudinal_acceleration = clip(
    solver_result.ax,
    flmpc_parameters_.ax_min,
    flmpc_parameters_.ax_max);

  longitudinal_velocity_command_ = clip(
    longitudinal_velocity_command_ +
    longitudinal_acceleration * flmpc_parameters_.sample_time,
    flmpc_parameters_.vx_min,
    flmpc_parameters_.vx_max);
...
// Read the current map -> base_link pose without requiring velocity feedback.
bool QCar2FLMPCController::readCurrentPoseFromTF(Pose2D & pose)
{
  ...
  std::vector<std::array<double, 2>> & input_reference_horizon) const
{
  ...
  state_reference_horizon.reserve(
    static_cast<std::size_t>(flmpc_parameters_.horizon_steps + 1));
  input_reference_horizon.reserve(
    static_cast<std::size_t>(flmpc_parameters_.horizon_steps));

  for (int offset = 0; offset <= flmpc_parameters_.horizon_steps; ++offset) {
    const std::size_t index = std::min(
      start_index + static_cast<std::size_t>(offset),
      active_trajectory_.samples.size() - 1U);
...
// Build, cache, and publish the fixed reference preview P0 -> ... -> PN.
void QCar2FLMPCController::publishReferencePreviewPath()
{
  ...
  if (!has_reference_path_msg_) {
    ...
    try {
      ...
      if (points.empty()) {
        RCLCPP_WARN(
          this->get_logger(),
          "Fixed reference preview path has no waypoints.");
        return;
      }

      const Pose2D preview_start_pose{
        points.front()[0],
        points.front()[1],
...
// Build, cache, and publish the active trajectory S0 -> ... -> P0 -> ... -> PN.
void QCar2FLMPCController::publishActivePath()
{
...
void QCar2FLMPCController::pathPublishTimerCallback()
{
  if (
    has_reference_path_msg_ &&
  ...
  if (
    has_active_path_msg_ &&
    active_path_publisher_ &&
...
// Stop tracking, export results, and clear FLMPC warm-start data.
void QCar2FLMPCController::stopTracking(const std::string & reason)
{
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "FLMPC Mk2 tracking stopped: %s. Waiting for a new 2D Goal trigger.",
    reason.c_str());
}
